...
class YoutubeVideoResult(tk.Frame):
    ...
    def create_video_result(self, words):
        ...
        self.result = tk.Label(self, text=words, font=f3, height=1, width=80, anchor='nw')

        self.table = ttk.Treeview(self, height=15, show="headings")
        self.table["columns"]=("影片名稱","觀看次數", "讚踩比", "留言數量", "上傳時間","影片連結")
        ...
        self.result.grid(row=0, column=0, columnspan=3, sticky = tk.W+tk.N, padx=(20,0))
    ...
    def select_item(self, event):  # 點選表格內文字
        ...
        if col in '#1#2#3#4#5#6':
            cell_value = cur_item['values'][5]  # 影片網址
